...
pub fn serializeJson(stream: anytype, comptime T: type, value: T) @TypeOf(stream).Error!void {
    try std.json.stringify(value, .{}, stream);
}

// pub fn serializeRecursiveJson(stream: anytype, comptime T: type, value: T, options: SerializeOptionsMaskFlags) @TypeOf(stream).Error!void {
...
//             switch (ptr.size) {
//                 .One => try serializeRecursiveJson(stream, ptr.child, value.*, options),
//                 .Slice => {
...
//                     } else {
//                         for (value) |item| {
//                             try serializeRecursiveJson(stream, ptr.child, item, options);
//                         }
//                     }
//                 },
//                 .C => unreachable,
...
//             } else {
//                 for (value) |item| {
//                     try serializeRecursiveJson(stream, arr.child, item, options);
//                 }
//             }
...
//                 try serializeRecursiveJson(stream, fld.type, @field(value, fld.name), options);
//             }
//         },
//         .Optional => |opt| {
//             if (value) |item| {
//                 try stream.writeInt(u8, 1, endian);
//                 try serializeRecursiveJson(stream, opt.child, item, options);
//             } else {
...
//                 try serializeRecursiveJson(stream, eu.payload, item, options);
//             } else |item| {
//                 try stream.writeInt(u8, 0, .little);
//                 try serializeRecursiveJson(stream, eu.error_set, item, options);
//             }
//         },
//         .ErrorSet => {
...
//             try serializeRecursiveJson(stream, Tag, active_tag, options);
...
//             try serializeRecursiveJson(stream, @TypeOf(array), array, options);
//         },
